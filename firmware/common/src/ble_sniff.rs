...
//! `AUX_CHAIN_IND`).
//!
//! Capture and decode run in separate tasks. [`scan`] copies each packet into
//! [`RX_QUEUE`] and immediately re-arms the radio; [`log_task`] drains the queue
//! and does the decoding and formatting while the radio is listening again.
...
use crate::common::{
    arm_rxen_after, csa2_chan_id, csa2_channel, data_ch_freq, disarm_rxen, radio_disable_silent,
    radio_ensure_disabled, set_access_address, set_pcnf0, set_pcnf0_coded, ADV_AA, ADV_CRC_INIT,
};
...
/// Sets the RX ramp-up to 40 µs (`MODECNF0.RU = Fast`) for the sniffer.
///
/// [`scan`] re-arms the receiver on every packet, so the ramp is paid once per
/// channel visit — a median 1.82 ms in the 2026-07-31 captures, making the ramp
/// 2.2% of it. Measured against the 140 µs ramp over two 2026-07-31 runs, the
/// visit shortens by the full 100 µs and the capture rate goes 522 → 550 pkt/s.
///
/// Every reception on this path begins at `TASKS_RXEN` and starts sampling at
/// READY through the `RXREADY_START` short, so the ramp's one effect here is how
/// soon READY arrives. (`gatt.rs` pins `RU` for its own path, where the hardware
/// T_IFS counter times the turnaround against the ramp.)
///
/// One call per boot covers the mode: `MODECNF0` holds its value across the
/// `MODE`/`PCNF0` writes [`follow_aux`] makes for a 2M- or Coded-PHY aux.
...
async fn follow_aux(first: decoder::AuxPtr, adi: Option<u16>, mut t_ref: Instant) {
    ...
    while let Some(aux) = next {
        ...
        //
        // Silent, not `radio_ensure_disabled`: the primary scan is still in RX
...
async fn follow_periodic(si: decoder::SyncInfo, t_anchor: Instant, phy: u8) {
    ...
    let chan_id = csa2_chan_id(si.aa);
    ...
    while events < PSYNC_MAX_EVENTS && miss < PSYNC_MAX_MISS {
        let (ch, freq) = match csa2_channel(counter, chan_id, &si.chm) {
            Some(cf) => cf,
...
pub async fn scan(rng: &mut Rng) {
    ...
    for &(ch_idx, freq) in order.iter() {
        ...
        while Instant::now() < deadline {
            ...
            let has_addr    = crc_ok && !is_ext && length >= 6;
            let payload_len = (length as usize).min(buf.len().saturating_sub(2));
            ...
            // fp: low 16 bits of the payload fingerprint — a stable handle for a
            // device across RPA rotation (the address changes, this does not).
            let fp = if crc_ok && has_addr {
                fnv1a(&buf[2..2 + payload_len]) as u16
            } else {
                0
            };
...
/// Drains [`RX_QUEUE`], decoding and logging one packet at a time.
///
/// Runs while the radio is listening, so the decode cost overlaps reception.
/// `with_log_stamp` gives every line the packet's air time, keeping the log a
/// record of when packets aired.
#[embassy_executor::task]
pub async fn log_task() -> ! {
    let mut sink = crate::LogSink;
    loop {
        let p = RX_QUEUE.receive().await;
        let d = Decoded { hdr: Header::parse(&p), pkt: &p };
        crate::with_log_stamp(p.t_air, || d.write_text_to(&mut sink));
    }
}
    ...
    if let Some(ad) = h.addr {
        ...
        if tx_add != 0 && a[5] >> 6 == 0b01 {
            ...
            for (irk, label) in crate::keys::IRKS {
                if crate::crypto::ah(irk, &prand) == hash {
        ...
        match salvage(buf) {
            Some(t) => {
                ...
                if matches!(t, 0x00 | 0x02 | 0x04 | 0x06) && length > 6 {
                    ...
                    let ad = &buf[8..];
                    unparsed = tail(6, decoder::log_ad_structures(ad, Some(adva)), ad.len());
                }
            }
            None => {
