...
//!
//! When the advertising sniffer ([`crate::ble_sniff`]) catches a `CONNECT_IND`,
...
//! dumped raw on `ct+` lines — 128 bytes to a line, since there is nothing in it
//! to line offsets up against and a 251-byte PDU in the annotated form is 16
//! lines. The bytes are kept because they are the whole record of the link from
//! that point and offline decryption needs them.
...
use crate::common::{
    arm_rxen_after, csa2_chan_id, csa2_channel, data_ch_freq, disarm_rxen, radio_configure_ble,
    radio_ensure_disabled, set_access_address, set_pcnf0, ADV_CRC_POLY,
};
...
// following in `ble_sniff`. See `common::csa2_channel` / `csa2_chan_id`.
...
///
/// Re-arms the advertising AA/CRC via [`radio_configure_ble`] at entry, which
...
pub async fn follow(
    ...
    // either way.
    let chan_id = csa2_chan_id(spec.aa);
    ...
    // Set once the encryption start-up handshake completes; from then on every
    // captured payload is ciphertext. Per connection, so it clears on the next.
    ...
    loop {
        ...
        // This event's channel. CSA#2 reads it straight off the event counter;
        // CSA#1 walks its unmapped index one hop forward, which is why that
        // index is also advanced for every event the catch-up above skipped.
        let selected = if spec.csa2 {
            csa2_channel(ev as u16, chan_id, &chm)
        } else {
            unmapped = (unmapped + hop) % 37;
        ...
        let mut act = capture(DIR_C2P, ev, crc_m, air_start, RX_M.0.get(), &mut pkt_no, &mut encrypted);
        if got_s {
            ...
            let s_act = capture(DIR_P2C, ev, crc_s, s_air, RX_S.0.get(), &mut pkt_no, &mut encrypted);
...
fn capture(
    ...
    *pkt_no += 1;
    ...
    if crc_ok && hdr.llid == 0b11 {
        ...
        // when one of them is missed. LL_ENC_REQ (0x03) is excluded: the 0x04
        // reply that follows it still carries a plaintext SKD/IV worth decoding.
        if matches!(b[2], 0x04..=0x06) {
            *encrypted = true;
        }
        action
    } else {
