...
//! hash built on it.
//!
//! Used only to resolve rotating identities (RPA for #8, CSIP RSI for #9)
//! against compiled-in keys — see [`crate::keys`] — so the whole module is gated
//! behind the `resolve-identities` Cargo feature and is absent from the default
//! build.
