...
//! [`crate::hal::radio::configure_154`]. It cannot run alongside the BLE modes:
...
//!
//! Structure mirrors [`crate::mode::ble_sniff`]: [`scan`] captures and copies into
...
//! in [`crate::mode::ble_sniff::scan`]); it also takes `Peri<RADIO>` ownership, which
...
use crate::hal::radio::{disable_silent, ensure_disabled, use_fast_ramp_up, zb_ch_freq};
...
struct Blink<'a, S: led::OnBoardLed> {
    leds: &'a mut S,
    off_at: Option<Instant>,
}

impl<S: led::OnBoardLed> Blink<'_, S> {
...
/// Bound on the ED wait, in the same spirit as [`crate::hal::radio::wait_disabled`]:
...
pub async fn scan(rng: &mut Rng, leds: &mut impl led::OnBoardLed) {
...
async fn dwell_channel(ch: u8, freq: u8, ms: u64, rng: &mut Rng, led: &mut Blink<'_, impl led::OnBoardLed>) {
    let r = pac::RADIO;
...
// ── Mode ──────────────────────────────────────────────────────────────────────
//
// The Zigbee-sniff boot mode: an 802.15.4 survey (energy sweep → dwell on occupied
// channels), forever. `init` reconfigures the radio for 802.15.4. The RGB LED is
// flashed **inline** in [`scan`] (green sweep / red channel-change / blue frame),
// so the mode holds a [`Gpio`] and `led_control` is a no-op; the decode consumer is
// the separate [`log_task`] the binary spawns.

/// Holds the [`Gpio`] LED it flashes per event; `K` names the sink type for trait
/// uniformity (unused — the decode consumer is a separate task).
pub struct ZigbeeSniff<K: super::CaptureSink> {
    leds: Option<Gpio>,
    _k: PhantomData<K>,
}

impl<K: super::CaptureSink> ZigbeeSniff<K> {
    pub fn new(leds: Gpio) -> Self {
        Self { leds: Some(leds), _k: PhantomData }
    }
}

impl<K: super::CaptureSink> Mode for ZigbeeSniff<K> {
    type Sink = K;

    async fn init<F: core::future::Future<Output = ()>>(&mut self, _ctx: &'static Ctx<K>, setup: F) {
        // Build-specific plumbing (spawns the decode consumer / LED task).
        setup.await;
        // Undo the BLE radio setup every boot does before the mode is known.
        crate::hal::radio::configure_154();
    }

    async fn run(&mut self, ctx: &'static Ctx<K>) -> ! {
        let mut leds = self.leds.take().expect("run once");
        // RX-only, so the shorter ramp is safe (no T_IFS turnaround to miss).
        use_fast_ramp_up();
        loop {
            scan(ctx.rng(), &mut leds).await;
        }
    }

    async fn led_control<L: OnBoardLed>(_led: &mut L) -> ! {
        // The LED is flashed inline in `run`, per event — nothing separate.
        pending().await
    }
}
