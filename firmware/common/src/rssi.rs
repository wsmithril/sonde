...
use crate::common::radio_ensure_disabled;
...
fn sample_rssi(freq_offset: u8) -> u8 {
    let r = pac::RADIO;
    radio_ensure_disabled(); // guard against leftover radio state
    r.frequency().write(|w| {
...
pub async fn sweep(spi: &mut Spim<'static>, rng: &mut Rng, leds: &mut impl crate::led::Sink) {
