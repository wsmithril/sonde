...
//!
//! Everything the mode logic needs lives here: the radio capture paths
//! (`ble_sniff`, `conn_follow`, `gatt`, `zb_sniff`, `rssi`), the decoders, and the
//! shared prelude below (the log channel + macros, `SyncBuf`, `Rng`). Each binary
//! crate supplies the pieces that differ — the USB CDC console vs. the SD PCAP
//! sink — plus its own `#[embassy_executor::main]`, interrupt bindings and
//! peripheral setup.
...
/// [`common::radio_ensure_disabled`] recovers it in place with no data lost, so
...
/// [`common::radio_ensure_disabled`] disabled it in place. Each one is a benign
