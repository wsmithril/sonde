...
fn main() {
    ...
    // ── Personal carry-device blacklist (optional config) ────────────────────
    // `blacklist.conf` holds the device names to exclude from probing; it is
    // gitignored (personal) and may be absent — a missing file means an empty
    // blacklist and the build still succeeds. The file's text is embedded into
    // the crate as a Rust string literal (`blacklist_conf.rs`).
    let conf = fs::read_to_string("blacklist.conf").unwrap_or_default();
    fs::write(
        out.join("blacklist_conf.rs"),
        format!(
            "// Generated by build.rs from blacklist.conf (optional) — do not edit.\n\
             pub const BLACKLIST_CONF: &str = {conf:?};\n"
        ),
    )
    .unwrap();
    println!("cargo:rerun-if-changed=blacklist.conf");
