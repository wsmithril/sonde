...
bare-metal Rust on Embassy. It listens to the traffic already in the air and
reports what it finds. Four modes cover Bluetooth Low Energy and one covers
**IEEE 802.15.4** (Zigbee/Thread); four are **passive** (transmit nothing,
undetectable) and one is an **active central** that connects and interrogates
devices. Modes are selected by reset-cycling the board — no reflash.

Two builds share the code: `sonde-usb` logs decoded ASCII over a USB serial
console, and `sonde-headless` is a drop-and-collect probe that writes PCAP/text
to an SD card. The firmware drives the RADIO directly (no SoftDevice), which is
what makes the passive modes possible.

## Verified nRF52840 dev kits that Sonde supports

This project has been tested on two nRF52840 dev kits:

- The official XIAO nRF52840, and its Plus variant.
  Sensor variants are also supported, but for now those sensors are not used.
- The NoLogo Super nRF52840 v1.5, a dev kit that is pin2pin compatible with the XIAO nRF52840 and much
  cheaper. But beware that some resellers, most notably the official NoLogo Taobao shop,
  do not populate the board with 2 MB flash and the LiPo charging IC.

Testing shows that the NoLogo kit has a higher CRC error rate when sniffing BLE adv packets,
11% for the XIAO kit vs 23% for the NoLogo kit, possibly due to a lower-quality chip antenna.
But somehow writing to the QSPI via a serial tty is much faster.
...
firmware/   Cargo workspace, thumbv7em — one shared lib + two binaries
  common/   `sonde_common`: everything both builds share
    build.rs    parses ../../assets/ into internal search indices + the QSPI image
    memory.x    linker script; app region ends at 0xEC000
    src/
      lib.rs       prelude: LOG channel + log macros, SyncBuf, Rng, render Sink
      mode/        one self-contained module per capture mode; the Mode/Ctx/
                   CaptureSink/Frame traits and the shared LED indicator live here
        ...
        zigbee.rs      passive IEEE 802.15.4 survey: energy sweep + MAC decode
        midea.rs       active central specialised to Midea appliances
      hal/         low-level substrate the modes share: radio.rs (BLE/802.15.4
                   RADIO setup, whitening, CRC), csa2.rs, hash.rs, crypto.rs
      led.rs       onboard RGB LED primitives (PWM/GPIO, colours)
      decoder/     AD parsing, 45 vendor decoders, hexdump, asset lookups
  usb/      `sonde-usb`: USB CDC console + boot-mode dispatch (main.rs, led.rs)
  headless/ `sonde-headless`: SD/PCAP capture + USB MSC (main.rs, storage/)
...
- **Hardware** — both builds run on a Seeed XIAO nRF52840 with the Adafruit UF2
  bootloader. The RSSI monitor additionally expects a 64-pixel WS2812 strip; the
  other modes need nothing beyond the board. `sonde-headless` adds an SD card on
  SPI (P1.12–15) — see below.

`sonde-headless` is built for a different job: a **drop-and-collect** probe you
leave running on a battery, then retrieve and read back later. Same XIAO board and
radio as `sonde-usb`; it just drops the QSPI name tables (decode happens later, on
the host) and adds a commodity SD card for storage (and optionally a GPS + RTC for
real timestamps). Both builds share the RGB LED and its implementation.
