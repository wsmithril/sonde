...
# SD-capture build). `device` is the serial port of the board in bootloader mode
# (default: the first /dev/tty.usbmodem* on macOS, /dev/ttyACM* on Linux).
# Double-tap reset to enter the bootloader first.
#
...
#
# Steps: objcopy the release ELF to a raw binary, wrap it in a DFU package, then
# stream it to the board. Override the objcopy binary with $OBJCOPY if needed.
set -e
...
adafruit-nrfutil dfu serial -pkg sonde_dfu.zip -p "$DEV" -b 115200
