...
Both builds (§1) target the **same board, the XIAO nRF52840**; they differ only in
what they do with a capture (console vs SD), not in hardware.
...
**Two builds, one board.** The workspace is a shared library plus two binary
crates. Both target the **Seeed XIAO nRF52840**; they package the same
capture/decode code for two *uses*:

- **`sonde-usb`** (`firmware/usb`) — decoded ASCII console over USB CDC, QSPI name
  tables, provisioning, the RGB LED.
- **`sonde-headless`** (`firmware/headless`) — battery **drop-and-collect** capture
  to an SD card (SPI on P1.12–15) as PCAP/text with a read-only FAT32 view over
  USB-MSC; no console, no QSPI, no deep decode (dead-code-eliminated). Decode moves
  to the host, off the raw capture, so it can be scattered and read back later.

Both are the same nRF52840 with the Adafruit UF2 bootloader, so `memory.x` (app at
`0x27000`), the reset-cycled boot mode, and the **onboard RGB LED** (P0.26/P0.30/
P0.06) are shared. `firmware/common` is board-agnostic; the capture modes drive an
`impl led::Sink`/`ChanSink` rather than a concrete LED, and the LED backend itself
now lives in `common/src/led.rs` — one implementation for both binaries (see
below). The only real difference is the SD card + USB-MSC the headless build adds.
...
### Capture modes as a trait (`common/src/mode/`)

Every capture mode is a struct in `common/src/mode/<name>.rs` implementing the
`Mode` trait, so the two binaries reduce to "pick the struct for
`next_boot_mode`'s choice, spawn its `run` and LED tasks." The data flow is
**radio → sink → decode → phy output**:

- **`Mode`** — `type Sink: CaptureSink` (the one output backend, per build);
  `async fn init(&mut self, &'static Ctx)`; `async fn run(&mut self, &'static Ctx)`;
  `async fn led_control<L: OnBoardLed>(led)`. `run` and `led_control` are static so
  they alias nothing and each spawns as its own task; the mode module also exposes a
  concrete `led_task(Pwm)` that delegates to `led_control` (an Embassy `#[task]`
  can't be a trait method).
- **`Ctx<K: CaptureSink>`** — a `static` (per binary) holding the RNG and the sink
  behind the crate's `UnsafeCell` + `unsafe impl Sync` idiom, passed by `&'static`
  ref. `rng`/`sink` live in separate cells so a mode's two `&mut` never alias.
  Because it's `'static`, `run` is spawnable — no borrowing `main`'s locals.
- **`CaptureSink`** — the build's backend: `sink_frame<F: Frame>` and `sink_text`.
  The USB build (`ConsoleSink`) decodes/prints to the console; the headless build
  (`PcapSink`) PCAP/text-encodes to the SD ring. This is the "decode → phy" half.
- **`Frame`** — a captured packet carries its own decode (`decode_to<S: Sink>`,
  typed and lossless) plus its PCAP fields. `RxPacket` implements it; ConnFollow
  and Zigbee keep their own typed `log_task` decoders (not routed through the sink).

BleSniff/GattEnum/MideaCtl are ZSTs (state is in `Ctx`); ConnFollow/Zigbee hold
their `Gpio` (inline LED) and Rssi holds SPI+PWM.

### LED indication (`common/src/led.rs`, one implementation)

Both builds run on the XIAO's one common-anode RGB LED — P0.26 (R), P0.30 (G),
P0.06 (B), every channel **active-LOW** — so the whole LED implementation lives
**once** in `common/src/led.rs` and both binaries re-export it. It holds the colour
model (`Rgb`/colours, `Rgb::mix`), the `led::LED` command signal, two sink traits
`OnBoardLed` (`set`) and `ChanSink: OnBoardLed` (`set_chan`) — the render sink is a
separate `crate::Sink`, hence the explicit name — the two backends, and
`fatal_blink`. A mode drives an `impl OnBoardLed`, never a concrete LED.
...
- **`led::Pwm`** — three PWM channels, 8 bits per channel, any mix. Each update is
  ...
  dropping it is safe and keeps the executor free for the radio (§4).
- **`led::Gpio`** — three plain outputs, the eight corners of the cube, one
  register write per channel and no DMA. The fit for the boot indicator (runs
  before the executor) and for connection-follow (toggles the LED between events).
...
brightness. `Pwm::set` squares the channel value (perceived brightness goes as
duty²); crossfades (`Rgb::mix`) interpolate before the gamma, so it applies once.

**Per-mode LED behaviour lives in the mode, not in `led`.** `mode::ble_sniff` owns
the rate/liveness indicator (`drive_led`); `mode::drive_indicator` renders the
`led::LED` state-colour signal for GATT/Midea (`led_control` + `led_task`);
ConnFollow/Zigbee drive a `Gpio` inline per radio event (`led_control` is a stub);
Rssi drives the strip + LED inside its sweep. `led.rs` stays board primitives only.
...
`hal::radio::configure_ble` holds the shared setup — BLE PHY, whitening, CRC,
channel maths — for sniff, rssi, gatt and conn_follow. `radio::configure_154`
...
`radio::configure_ble` sets `MAXLEN = 255` deliberately, because a large
...
All three steps are load-bearing, and `hal::radio` is the only place that gets to
do them. `radio::ensure_disabled` (loud — a running radio is a bug here) and
`radio::disable_silent` (quiet — a running radio is expected) both route through
one `force_disable`.
...
### Opaque advertising vendors

Some high-volume manufacturer beacons carry no decodable plaintext and are left as
a raw hex dump on purpose — the payload is a rotating/encrypted token with no
public field layout, so a decoder would only invent structure. These are noted
here so they are not re-investigated:

- **U-tec / ULTRALOQ (`0xFF01`)** — ~990k packets in a single home capture, the
  largest single opaque vendor seen. The advert is a rolling **presence** beacon;
  lock state and control live behind an **authenticated, encrypted GATT**
  connection (`ADMIN_LOGIN` + AES). The keys are **per-device secrets fetched from
  the U-tec cloud** (email/password login → `cloud.u-tec.com/app/device/list`), not
  derived locally from MAC/UID and with no static/global key — confirmed against
  `maeneak/utecio` (the library behind `inventor7777/ultraloq-ble-ha`, 2026-08-10).
  So the adverts are undecodable passively, and even active control would need the
  owner's cloud credentials with the lock enrolled — it can't decrypt a captured
  session or a stranger's lock. That library is a useful reference only for a
  *control-my-own-lock* mode, not for sniff decode.
- **Others in the same class** (no public plaintext layout): `0xFFFF` SIG-reserved
  misuse, Google FCF1/FEF3 (Play Services), Midea `0x06A8`. See also the general
  [encrypted-payloads](#encrypted-payloads) note for the connected-link case.

A packet whose first AD-structure length byte is nonsensical for its payload size
(e.g. `0x6F` in a 30-byte payload) is not standard AD at all — a proprietary or
encrypted beacon — and the walker correctly falls through to the raw dump rather
than mis-parsing it.
...
Shared radio code lives in `hal::radio`. GATT/ATT decode stays in `mode::gatt`:
...
## 8. Connection follow (`mode::conn_follow`)
...
## 9. IEEE 802.15.4 sniff (`mode::zigbee`)
...
### `radio::configure_154` — the four settings that silently deafen the receiver

Register values transcribed from `embassy-nrf`'s own
`radio/ieee802154.rs` and checked against PS v1.11. Most of the block is
unsurprising; four fields differ from `radio::configure_ble` in ways that produce
